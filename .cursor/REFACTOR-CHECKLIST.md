...
- [ ] 重构 >200 行代码
- [ ] 重构 >5 个文件
- [ ] 重构核心模块（HealthKit / Firebase / SwiftData）
- [ ] 修改数据模型关系
- [ ] 修改三大铁律相关代码
...
// 三大铁律（绝不能违反）
1. HealthKit 只读（不能删除/修改 HKWorkout）
2. UESCA 原则（训练计划生成逻辑）
3. 性能指标（启动<2s, AI反馈<5s, SwiftData<100ms）
```
...
# 性能对比（重构前后）
# 1. 启动时间（用 Instruments 测量）
# 2. SwiftData 查询耗时（添加 Logger.log()）
# 3. 内存占用（Xcode Memory Debugger）

# 检查命名规范（重构后）
find Epoch/Models -name "*.swift" | grep -v "Cached"
find Epoch/DTOs -name "*.swift" | grep -v "DTO\.swift"
```
